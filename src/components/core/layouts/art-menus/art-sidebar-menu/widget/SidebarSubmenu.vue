<template>
  <template v-for="(item, index) in filteredMenuItems" :key="getUniqueKey(item, index)">
    <ElSubMenu v-if="hasChildren(item)" :index="(item.path || item.meta.title || '') as string" :level="level">
      <template #title>
    ...
    <ElMenuItem
      v-else
      :index="(isExternalLink(item) ? undefined : item.path || item.meta.title || '') as string | undefined"
      :level-item="level + 1"
      @click="goPage(item)"
    >
      <div class="menu-icon flex-cc">
